...
class GoogleAPIClient:
    ...
    async def get_message_async(self, message_id: str) -> Dict[str, Any]:
        """Get a single Gmail message by ID (async)."""
    ...
    async def get_thread_async(self, thread_id: str) -> Dict[str, Any]:
        """Get a Gmail thread by ID with all messages (async)."""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(
            None,
            lambda: (
    ...
    async def search_messages_async(
        ...
        """Search Gmail messages with pagination (async)."""
        # Add inbox filter if not specified
        if "in:inbox" not in query.lower() and "in:" not in query.lower():
            query = f"{query} in:inbox" if query.strip() else "in:inbox"
        ...
        results = await loop.run_in_executor(
            None,
            lambda: self._gmail.users().messages().list(**params).execute()
        )
        messages = results.get("messages", [])
        next_token = results.get("nextPageToken")
    ...
    async def get_event_async(self, event_id: str, calendar_id: str = "primary") -> Dict[str, Any]:
        """Get a single calendar event by ID (async)."""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(
            None,
            lambda: (
    ...
    async def search_events_async(
        ...
        results = await loop.run_in_executor(
            None,
            lambda: self._calendar.events().list(**params).execute()
        )
        events = results.get("items", [])
        next_token = results.get("nextPageToken")
    ...
    async def search_contacts_async(self, query: str) -> List[Dict[str, Any]]:
        """Search contacts using People API (async)."""
        loop = asyncio.get_event_loop()
        results = await loop.run_in_executor(
            None,
            lambda: (
    ...
    async def get_document_async(self, document_id: str) -> Dict[str, Any]:
        """Get a Google Docs document by ID (async)."""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(
            None,
    ...
    async def get_file_metadata_async(
        ...
        """Get Google Drive file metadata (async)."""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(
            None,
    ...
    async def get_file_revisions_async(self, file_id: str) -> List[Dict[str, Any]]:
        """Get all revisions for a Google Drive file (async)."""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(
            None,
    ...
    async def get_latest_revision_id_async(self, file_id: str) -> Optional[str]:
        """Get the latest revision ID for a Google Drive file (async)."""
        try:
            revisions = await self.get_file_revisions_async(file_id)
            if revisions:
                # Revisions are returned in chronological order, so the last one is the latest
                return revisions[-1].get("id")
            return None
    ...
    async def check_file_revision_async(self, file_id: str, cached_revision_id: str) -> bool:
        """Check if the cached revision ID matches the current latest revision (async).
        ...
        """
        try:
            current_revision_id = await self.get_latest_revision_id_async(file_id)
            return current_revision_id == cached_revision_id
        except Exception:
            # If we can't check, assume it's invalid to be safe
            return False
    ...
    async def search_documents_async(
        ...
        """Search Google Docs documents with pagination using flexible parameters (async)."""
        # Start with base query for Google Docs
        drive_query_parts = [
            "mimeType='application/vnd.google-apps.document'",
            "trashed=false",
        ...
        results = await loop.run_in_executor(
            None,
            lambda: self._drive.files().list(**api_params).execute()
        )
        files = results.get("files", [])
        next_token = results.get("nextPageToken")
